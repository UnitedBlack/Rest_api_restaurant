...
@router.get("/menus/{menu_id}", tags=["specified_menu"])
...
@router.patch("/menus/{menu_id}", tags=["change_menu_data"])
async def change_menu_data(
    menu_id: UUID, data: RestaurantDTO.Menu = None, db: Session = Depends(get_db)
):
...
async def delete(id: UUID, db: Session = Depends(get_db)):
    return database_service.remove(db, id)
